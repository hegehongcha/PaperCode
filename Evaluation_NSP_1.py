...
def main():
    ...
    while epoch <= 20:
        ...
        try:
            checkpoint = torch.load(f"./{config.datasets}/Evaluation_NSP_no_rec.pkl")
            first = 0
            print("111")
        except:
            first = 1
        ...
        if flag:
            ...
            with open(f"./{config.datasets}/Evaluation_NSP_no_rec.json", "w", encoding="utf-8") as f:
                f.write(rows_json)
            print(555)
        else:
            rows_index = checkpoint["rows_index"]
            ...
            with open(f"./{config.datasets}/Evaluation_NSP_no_rec.json", "r", encoding="utf-8") as f:
                ...
                if len(NSP_encoder_list) <= 512:
                    ...
                    if i % 16 == 0:
                        print("epoch:", epoch, "iter:", i, "loss:", loss.item(), "loss_kl:", loss_kl_item, "rec_loss:",
                              rec_loss_item, "NSP_loss", NSP_loss.item(), "rows:", rows_index, f"./{config.datasets}/Evaluation_NSP_no_rec")
                    i = i + 1
                    if i % 1000 == 0:
                        checkpoint = {"model_state_dict": model.state_dict(),
                                      ...
                                      "iter": i,
                                      "first": first}
                        torch.save(checkpoint, f"./{config.datasets}/Evaluation_NSP_no_rec.pkl")
