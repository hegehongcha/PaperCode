...
def mlog(s, config):
    ...
    with open(f"./{config.datasets}/log/MI_NSP_1.log", "a+", encoding="utf-8") as log_f:
        log_f.write(s+"\n")
    try:
        print(s)
    except:
...
def main():
    ...
    checkpoint = torch.load(f"./{config.datasets}/Evaluation_NSP.pkl")
    ...
    for m, batch in enumerate(s2s_samples["samples"]):
        B_hat_index_list = np.random.randint(0, len(texts)-1, 100)
        ...
        MI_calculate(x, B_hat_index_list, condition_text, posterior_r, "OPTIMUS", config, texts, model, label_text)
        NSP_eval(model, condition_text, x, "OPTIMUS", config)
        ...
        NSP_eval(model, condition_text, s2s_x, "s2s", config)
        ...
        NSP_eval(model, condition_text, hred_x, "hred", config)
main()