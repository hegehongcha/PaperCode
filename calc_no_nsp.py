...
def mlog(s, config):
    ...
    with open(f"./{config.datasets}/log/MI_NSP_no_nsp_sample.log", "a+", encoding="utf-8") as log_f:
        log_f.write(s+"\n")
    try:
        print(s)
    except:
...
def main():
    ...
    with open(f"./{config.datasets}/all_samples_same_personachat_4.json", "r", encoding="utf-8") as f:
    ...
    for m, batch in enumerate(s2s_samples):
        ...
        label_text = batch["reference"].strip()
        condition_text = batch["condition"].strip()
        log_s = \
        ...
        x = batch["generated_text"]
        prior_c = prior_z_encoder(model, condition_text)
        MI_1 = MI_calculate(x, B_hat_index_list, condition_text, posterior_r, batch["model"], config, texts, model, label_text)
        MI_2 = MI_calculate_1(x, B_hat_index_list, condition_text, prior_c, batch["model"], config, texts, model, label_text)
        # pred = NSP_eval(model, condition_text, x, "OPTIMUS", config)
        score = MI_1 + MI_2
        log_s = f"score: {score}\n"
        batch["score_no_nsp_train"] = float(score)
        mlog(log_s, config)
    ...
    with open(f"./{config.datasets}/all_samples_same_personachat_5.json", 'w', encoding='utf-8') as f:
