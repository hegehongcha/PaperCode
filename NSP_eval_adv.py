...
# def mlog(s, config):
#     if not os.path.exists(f"./dd++/log"):
#         os.makedirs(f"./dd++/log")
#
#     with open(f"./dd++/log/MI_NSP_adv_train.log", "a+", encoding="utf-8") as log_f:
#         log_f.write(s+"\n")
#     try:
#         print(s)
#     except:
...
def main():
    ...
    with open(f"./dd++/pre_test.json", "r", encoding="utf-8") as f:
        s2s_samples = json.load(f)

    # with open(f"./{config.datasets}/hred_eval_samples.json", "r", encoding="utf-8") as f:
    #     hred_samples = json.load(f)

    with open(f"./dd/train.txt", "r", encoding="utf-8") as fp:
        texts = fp.read().split('\n')

    num_sents = len(s2s_samples)
    num_accurate = 0
    for m, batch in enumerate(s2s_samples):
        # print(1111211)

        B_hat_index_list = np.random.randint(0, len(texts)-1, 10)
        # for B_hat_index in B_hat_index_list:

        label_text = batch["reference"]
        condition_text = batch["context"]
        # log_s = \
        ...
        s2s_x = batch["response"].strip()
        # MI_1 = MI_calculate(s2s_x, B_hat_index_list, conditions, posterior_r, batch["type"], config, texts, model, label_text)
        # MI_2 = MI_calculate_1(s2s_x, B_hat_index_list, condition_text, prior_c, batch["type"], config, texts, model, label_text)
        pred = NSP_eval(model, condition_text, s2s_x, batch["type"], config)
        # print(pred)
        if int(pred) == 1 and batch["type"] == "positive_responses":
            num_accurate += 1
        if int(pred) == 0 and batch["type"] != "positive_responses":
            num_accurate += 1
