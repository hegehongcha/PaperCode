...
def main():
    ...
    while epoch <= 40:
        ...
        try:
            checkpoint = torch.load("NLG_evaluation_2.pkl")
            first = 0
            print("111")
        except:
            first = 1
        ...
        if flag:
            ...
            with open("NLG_evaluation_2.json", "w", encoding="utf-8") as f:
                f.write(rows_json)
            print(555)
        else:
            rows_index = checkpoint["rows_index"]
            ...
            with open("NLG_evaluation_2.json", "r", encoding="utf-8") as f:
                rows_dict_str = f.read()
                rows_dict = json.loads(rows_dict_str)
            rows = rows_dict['rows']
        rows = np.array(rows)
        train_data_gen = Dataloader(text_path=r"positive_train_data.txt",
                                    batch_size=128,
                                    min_index=min_index,
                                    max_index=max_index,
        ...
        for train_data in train_data_gen:
            ...
            print("epoch:", epoch, "iter:", i, "loss:", loss.item(), "loss_kl:", loss_kl.item(), "rec_loss:",
                  rec_loss.item(), "rows:", rows_index, "NLG_evaluation_2")
            i = i + 1
            if i % 10 == 0:
                checkpoint = {"model_state_dict": model.state_dict(),
                              ...
                              "iter": i,
                              "first": first}
                torch.save(checkpoint, 'NLG_evaluation_2.pkl')
