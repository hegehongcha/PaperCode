...
def main():
    ...
    with open(f"./{config.datasets}/pre_train.json", "r", encoding="utf-8") as f:
        texts = json.load(f)
    ...
    while epoch <= 20:
        ...
        #                             rows=rows)
        train_data_gen = Dataloader(text_path=f"./{config.datasets}/pre_train.json",
                                    batch_size=32,
                                    min_index=min_index,
                                    max_index=max_index,
                                    rows_index=rows_index,
        ...
        for batch in train_data_gen:
            ...
            for train_data in batch[0]:
                sentence_a = train_data["context"]

                response = train_data["reference"]
                ...
                if train_data["type"] == "positive_responses":
                    sentence_b = train_data["response"]
                    label = 1
                else:
                    sentence_b = train_data["response"]
                    label = 0
